    ...
    Result PtpResponder::SendObjectPropList(PtpDataParser &rdp) {
        ...
        for (u32 i = 0; i < num_properties; i++) {
            ...
            switch (obj_property) {
                ...
                default:
                    {
                        /* Skip unknown property value based on its data type. */
                        if (type == PtpDataTypeCode_String) {
                            char skip_buf[PtpStringMaxLength + 1];
                            R_TRY(dp.ReadString(skip_buf));
                        } else {
                            const u16 base = static_cast<u16>(type & ~PtpDataTypeCode_ArrayMask);
                            R_UNLESS(base >= PtpDataTypeCode_S8 && base <= PtpDataTypeCode_U128,
                                     haze::ResultUnknownPropertyCode());
                            const u32 elem_size = 1u << ((base - 1) / 2);
                            ...
                            u32 dummy;
                            while (total > 0) {
                                const u32 n = std::min<u32>(total, sizeof(skip));
                                R_TRY(dp.ReadBuffer(skip, n, std::addressof(dummy)));
                                total -= n;
                            }
                        }
                    }
