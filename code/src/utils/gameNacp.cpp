 ...
 *
 * @param controlData 待检查并解压的 NACP 控制数据
 */
...
/**
 * @brief 获取游戏当前安装内容的版本号
 * @param appId 游戏 appId
 * @return 当前安装内容版本号，查询失败时返回 0xFFFFFFFE
 */
static uint32_t getGameVersion(uint64_t appId) {
    NsApplicationContentMetaStatus entries[2]{};
    s32 count = 0;

    Result rc = nsListApplicationContentMetaStatus(appId, 0, entries, 2, &count);
    if (R_FAILED(rc) || count <= 0) return 0xFFFFFFFE;

    s32 index = count >= 2 && entries[1].meta_type == NcmContentMetaType_Patch ? 1 : 0;
    return entries[index].version;
}
