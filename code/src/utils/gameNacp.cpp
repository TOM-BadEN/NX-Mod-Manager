...
GameMetadata getGameNACP(uint64_t appId) {
    ...
    Result rc;
    if (hosversionAtLeast(19, 0, 0)) rc = nsGetApplicationControlData2(NsApplicationControlSource_Storage, appId, controlData.get(), sizeof(NsApplicationControlData), 0xFF, 0, &jpegSize, nullptr);
...
std::string getVersion(uint64_t appId) {
    ...
    Result rc = nsGetApplicationControlData(NsApplicationControlSource_Storage, appId, controlData.get(), sizeof(NsApplicationControlData), &jpegSize);
    std::string version;
    if (R_SUCCEEDED(rc)) version = controlData->nacp.display_version;
    return version;
}
