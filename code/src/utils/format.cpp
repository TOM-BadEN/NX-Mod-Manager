...
bool parseIsoTimestamp(const std::string& dateString, std::time_t& target) {
    ...
    int parsedChars = 0;
    int parsed = std::sscanf(dateString.c_str(), "%d-%d-%d%*c%d:%d:%d%n", &year, &month, &day, &hour, &minute, &second, &parsedChars);
...
std::string themedIconPath(const std::string& path) {
    bool isDarkTheme = brls::Application::getThemeVariant() == brls::ThemeVariant::DARK;
    return path + (isDarkTheme ? "-dark.png" : "-light.png");
}
