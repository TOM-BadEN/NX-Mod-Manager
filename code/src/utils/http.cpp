...
struct ThreadCurl {
    CURL* handle = nullptr;     // 当前线程复用的 easy handle
    bool registered = false;    // 是否已加入全局 slot 列表
    ~ThreadCurl();
};
...
struct StreamData {
    const std::function<bool(const uint8_t* data, size_t size)>* callback; // 流式数据回调
};
...
struct ProgressData {
    const std::function<bool(size_t total, size_t now)>* callback; // 进度回调
...
static int cancelCallback(void* userdata, curl_off_t /*dltotal*/, curl_off_t /*dlnow*/, curl_off_t /*ultotal*/, curl_off_t /*ulnow*/) {
