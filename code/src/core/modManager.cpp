...
// 从所有已安装 MOD 收集涉及的 TID（去重），缺失字段时回退到游戏 TID
std::vector<std::string> ModManager::collectInstalledTidDirs() {
    std::string gameTidDir = format::appIdHex(m_game.appId);
    std::vector<std::string> allTidDirs;
    for (const auto& mod : m_mods) {
        if (!mod.isInstalled) continue;
        auto tidDirs = m_modJson.getStringArray(mod.dirName, "installedTidDirs");
        if (tidDirs.empty()) tidDirs.push_back(gameTidDir);
        for (auto& tidDir : tidDirs) {
            bool found = false;
            for (const auto& existing : allTidDirs) {
                if (existing == tidDir) { found = true; break; }
            }
            if (!found) allTidDirs.push_back(std::move(tidDir));
        }
    }
    return allTidDirs;
}
...
ModInstaller::InstallResult ModManager::installMod(int index,
    ...
    auto result = ModInstaller::install(m_mods[index], m_game, m_mods, progressCb, token);
    if (result.success) {
        m_mods[index].isInstalled = true;
        m_modJson.setBool(m_mods[index].dirName, "installed", true);
        m_modJson.setStringArray(m_mods[index].dirName, "installedTidDirs", result.installedTidDirs);
        m_modJson.save();
    }
    return result;
}
...
ModInstaller::UninstallResult ModManager::uninstallMod(int index,
    ...
    auto result = ModInstaller::uninstall(m_mods[index], m_game, progressCb);
    if (result.success) {
        m_mods[index].isInstalled = false;
        m_modJson.setBool(m_mods[index].dirName, "installed", false);
        m_modJson.removeKey(m_mods[index].dirName, "installedTidDirs");
        m_modJson.save();
    }
    return result;
}
...
bool ModManager::disableMods() {
    ...
    auto allTidDirs = collectInstalledTidDirs();

    // rename contents/{tid} → contents/{tid}-disable
    for (const auto& tidDir : allTidDirs) {
    ...
    // 遍历所有 mod，rename .ips → .ips-disable
    for (const auto& mod : m_mods) {
...
bool ModManager::enableMods() {
    ...
    // 遍历所有 mod，rename .ips-disable → .ips
    for (const auto& mod : m_mods) {
        std::string ipsDir = ModInstaller::atmospherePath + "/exefs_patches/" + mod.dirName + "_" + gameDirName;
        if (!fs::dirExists(ipsDir)) continue;
    ...
    std::function<void(int deleted, int total, const char* fileName)> onProgress)
{
    ...
    for (auto& mod : m_mods) {
        ...
        m_modJson.removeKey(mod.dirName, "installedTidDirs");
    }
    m_modJson.save();
