...
int StoreGameManager::findByTid(const std::string& tid) const {
    for (size_t index = 0; index < m_storeGameList.size(); index++) {
        if (m_storeGameList[index].gameTid == tid) return static_cast<int>(index);
    }
    return -1;
}
