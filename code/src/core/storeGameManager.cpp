...
int StoreGameManager::getCachedIcon(const std::string& tid) const {
    auto it = m_iconCache.find(tid);
    return it == m_iconCache.end() ? 0 : it->second;
}

void StoreGameManager::cacheIcon(const std::string& tid, int iconId) {
    if (tid.empty() || iconId == 0) return;
    m_iconCache[tid] = iconId;
}

const std::unordered_map<std::string, int>& StoreGameManager::iconCache() const {
    return m_iconCache;
}
