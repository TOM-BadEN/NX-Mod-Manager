...
// 模块级状态
static bool s_active = false;               // 是否已有实例在运行
static MtpManager::EventCallback s_onEvent; // 调用方注册的事件回调
static haze::FsEntries s_fsEntries;         // libhaze 要求的 proxy 列表
static long long s_pendingFileSize = 0;     // 当前传输文件的字节数

/**
 * @brief libhaze 文件系统代理
 *
 * 将每个 MTP 相对路径拼接到对应的 SD 卡目录，再转发给 Switch 文件系统。
 */
class MtpProxy : public haze::FileSystemProxyImpl {
public:
    // basePath 是 SD 卡目录，displayName 是 PC 盘符名，name 是 libhaze 内部存储 ID。
    ...
    /** @brief 获取 SD 卡文件系统是否可用 */
    bool isValid() const {
        return m_fileSystem != nullptr;
    }

    /** @brief 获取 libhaze 内部存储 ID */
    const char* GetName() const override {
        return m_name.c_str();
    }

    /** @brief 获取 PC 上显示的盘符名称 */
    ...
    // size 是 PC 提供的文件总大小。保存给事件回调后，以 0 创建文件，避免预分配阻塞。
    ...
    // 读取前捕获文件大小供 Read 回调使用；MTP 单线程串行，不会与写入状态冲突。
...
private:
    ...
    std::string m_displayName;  // PC 上显示的盘符名称
    std::string m_name;         // libhaze 内部存储 ID
    FsFileSystem* m_fileSystem; // SD 卡文件系统句柄

    /**
     * @brief 将 libhaze 相对路径转换为 SD 卡完整路径
     * @param relativePath libhaze 提供的相对路径
     * @param output 完整路径输出缓冲区
     *
     * 第一个 proxy 的对象名可能仍带前导斜杠，需要先移除，避免拼出双斜杠路径。
     */
...
/** @brief 将 libhaze 回调转换为 MtpEventData 并转发给调用方 */
static void hazeCallback(const haze::CallbackData* data) {
    ...
    switch (data->type) {
        ...
        // PC -> Switch。文件大小由 CreateFile 捕获，WriteEnd 后清零。
        ...
        // Switch -> PC。文件大小由 GetFileSize 捕获，ReadEnd 后清零。
...
bool MtpManager::start(std::vector<MtpMount> mounts, EventCallback onEvent) {
    ...
    // 第一个 proxy 使用默认存储 ID，后续依次使用 "1"、"2" 等 ID。
    for (size_t i = 0; i < mounts.size(); i++) {
        std::string name = i == 0 ? "" : std::to_string(i);
        auto proxy = std::make_shared<MtpProxy>(std::move(mounts[i].basePath), std::move(mounts[i].displayName), std::move(name));
        if (!proxy->isValid()) {
            s_fsEntries.clear();
            s_onEvent = nullptr;
            return false;
