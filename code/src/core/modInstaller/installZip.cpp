...
InstallResult installFromZip(const ModInfo& mod, const GameInfo& game,
    ...
    auto dirsResult = utils::buildTargetDirs(zip.dirs(), tid);
    result.installedTidDirs = std::move(dirsResult.installedTidDirs);
    std::vector<std::string> writtenFiles;

    utils::CreateDirsResult dirResult = utils::createDirs(dirsResult.targetDirs);
    if (!dirResult.success) {
        utils::rollback(writtenFiles, dirsResult.targetDirs, progressCb);
    ...
    for (int i = 0; i < totalFiles; ++i) {
        ...
        if (modFiles[i].targetPath.empty()) {
            ...
            dirsResult.targetDirs.push_back(pchtxt.ipsDir);
...
UninstallResult uninstallZip(const ModInfo& mod, const GameInfo& game, std::function<void(const Progress&)> progressCb) {
    ...
    // 遍历 dirs，倒序清理空目录
    for (int i = static_cast<int>(dirsResult.targetDirs.size()) - 1; i >= 0; --i) {
        fs::deleteEmptyDir(dirsResult.targetDirs[i]);
    }
