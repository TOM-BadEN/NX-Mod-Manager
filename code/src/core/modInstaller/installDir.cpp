...
struct DirScanResult {
    ...
    std::vector<std::string> installedTidDirs;
};

DirScanResult scanDirMod(const std::string& modPath, const std::string& tid, bool includeFiles,
    ...
    auto dirsResult = utils::buildTargetDirs(rawDirs, tid);
    result.dirs = std::move(dirsResult.targetDirs);
    result.installedTidDirs = std::move(dirsResult.installedTidDirs);
    result.success = true;
    return result;
}
...
InstallResult installFromDir(const ModInfo& mod, const GameInfo& game,
    ...
    for (int i = 0; i < totalFiles; ++i) {
        ...
        int64_t diskCrc = crc::fromFile(file.targetPath.c_str(), buf.crc, crcBufSize);
        if (diskCrc < 0) continue;

        int64_t srcCrc = crc::fromFile(file.sourcePath.c_str(), buf.crc, crcBufSize);
        if (srcCrc < 0 || static_cast<uint32_t>(srcCrc) != static_cast<uint32_t>(diskCrc)) {
            ...
            result.conflictMod = utils::findConflictModName(file.targetPath, static_cast<uint32_t>(srcCrc), allMods, buf.crc, crcBufSize);
    ...
    refCount.save();
    result.success = true;
    result.installedTidDirs = std::move(scan.installedTidDirs);
    return result;
}
...
UninstallResult uninstallDir(const ModInfo& mod, const GameInfo& game,
    ...
    auto scan = scanDirMod(mod.path, tid, true, progressCb, {});
    ...
    for (int i = 0; i < totalFiles; ++i) {
        ...
        if (progressCb) progressCb({false, i + 1, totalFiles, fileName, 0, 0});
        if (refCount.decrement(file.targetPath)) fs::deleteFile(file.targetPath);
    }
