...
void AppUpdater::check(std::stop_token token, const std::string& localVersion) {
...
api::app::VersionHistoryResult AppUpdater::fetchVersionHistory(std::stop_token token) {
    return api::app::fetchVersionHistory(token);
}

api::ApiResult AppUpdater::download(std::stop_token token, std::function<bool(size_t total, size_t now)> progressCb) {
    std::string tmpPath = std::string(config::appUpdateDir) + "temp/update.nro";
    return api::app::download(tmpPath, progressCb, token);
}
