...
ModUpdateCheckResult checkModUpdates(const std::string& gameTid, const std::string& modsJson, std::stop_token token) {
    auto resp = http::postJson(url::mod::checkUpdates(gameTid), modsJson, token);
    if (!resp.ok()) {
        JsonResp json(resp);
        std::string message = json.getString("message");
        return {false, message.empty() ? brls::getStr("other/api/networkError") : message};
...
ScreenshotResult fetchScreenshot(const std::string& gameTid, int modId, int index, std::stop_token token) {
    // 优先镜像加速
    auto resp = http::get(url::mod::screenshotMirror(gameTid, modId, index), token);
    if (resp.ok() && !resp.body.empty()) return {true, "", std::move(resp.body)};
    if (token.stop_requested()) return {};

    // 镜像失败，回退源站
    resp = http::get(url::mod::screenshot(gameTid, modId, index), token);
    if (resp.ok() && !resp.body.empty()) return {true, "", std::move(resp.body)};
