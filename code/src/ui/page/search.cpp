...
Search::Search(const std::vector<std::string>& items, std::function<void(int)> onSelect)
    ...
    setHeader();
    setFooterBackgroundTheme("app/keyboardBg");
}

void Search::setHeader() {
    HeaderState headerState;
    headerState.setTitle(brls::getStr("page/search/pageTitle"));
    ShellState::setHeaderState(headerState);
}

void Search::onContentAvailable() {
    // +：在虚拟键盘和搜索结果之间切换焦点
    registerAction(brls::getStr("page/search/switchResult"), brls::ControllerButton::BUTTON_START, [this](brls::View*) {
...
void Search::switchToResults() {
    ...
    updateActionHint(brls::ControllerButton::BUTTON_START, brls::getStr("page/search/switchKeyboard"));
...
void Search::switchToKeyboard() {
    ...
    updateActionHint(brls::ControllerButton::BUTTON_START, brls::getStr("page/search/switchResult"));
    brls::Application::getGlobalHintsUpdateEvent()->fire();
}
...
void Search::updateResults(const std::string& keyword) {
    ...
    setActionAvailable(brls::ControllerButton::BUTTON_START, true);
