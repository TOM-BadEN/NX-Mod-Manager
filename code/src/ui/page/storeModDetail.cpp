...
void StoreModDetail::onContentAvailable() {
    ...
    ShellState::setSubtitle(brls::getStr("page/storeModDetail/notInstalled"));
    if (m_gameVersion != "...") ShellState::setSubtitle(brls::getStr("page/storeModDetail/localVersion", format::cleanVersion(m_gameVersion)));
    setupLeftCard();
    setupScreenshots();
    setupButtons();
    setupCommentGrid();
    ...
    registerAction("", brls::BUTTON_B, [this](...) {
        ...
        if (dynamic_cast<StoreModList*>(previousPage)) Page::popPage();
...
void StoreModDetail::setupButtons() {
    ...
    m_buttonLike->registerAction(brls::getStr("page/storeModDetail/btnLike"), brls::BUTTON_A, [this](...) {
        Audio::instance()->play(SoundEffect::Enter);
        if (m_detailLoaded) onLikeAction();
        return true;
    });
    m_buttonDislike->registerAction(brls::getStr("page/storeModDetail/btnDislike"), brls::BUTTON_A, [this](...) {
        Audio::instance()->play(SoundEffect::Enter);
        if (m_detailLoaded) onDislikeAction();
        return true;
    });
    m_buttonDownload->registerAction(brls::getStr("page/storeModDetail/btnDownload"), brls::BUTTON_A, [this](...) {
        Audio::instance()->play(SoundEffect::Enter);
        if (m_detailLoaded) onDownloadAction();
        return true;
    });
    m_buttonComment->registerAction(brls::getStr("page/storeModDetail/btnComment"), brls::BUTTON_A, [this](...) {
        Audio::instance()->play(SoundEffect::Enter);
...
void StoreModDetail::showCompleteDialog(const std::string& message) {
    ...
    CustomDialog::show(message, {
        ...
        {brls::getStr(backKey), [this, fromModList] {
            ...
            CustomDialog::close([this, popCount] { Page::popPages(popCount, PageAnimType::SlideFromRight); });
        }},
    });
}
