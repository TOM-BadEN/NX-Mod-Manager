...
brls::ScrollingFrame* s_scroll = nullptr; // 当前对话框的滚动区域
int s_atBoundary = 0;                     // 0=未到边界, 1=上, 2=下
...
struct DialogViews {
    brls::Box* box = nullptr;       // 对话框布局根容器
    brls::Box* body = nullptr;      // 自定义内容容器
    brls::Label* label = nullptr;   // 长文本标签
    LongPressButton* btn = nullptr; // 长按确认按钮
};
...
DialogViews createDialogViews() {
    ...
    auto* box = dynamic_cast<brls::Box*>(brls::View::createFromXMLResource("view/dialog/longPressDialog.xml"));
    if (!box) return {};
...
void setupAndOpen(DialogViews views, const std::string& btnLabel, float holdSeconds, std::function<void()> onConfirm, std::function<void()> bAction)
{
    auto* btn = views.btn;
    if (btn) {
