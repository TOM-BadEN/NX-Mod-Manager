...
KeyboardInput::KeyboardInput(std::function<void(std::string)> onConfirm, std::string placeholder, int maxLength) {
    inflateFromXMLRes("xml/view/dialog/keyboardInput.xml");
    ...
    // +：确认输入
    m_keyboard->registerAction(brls::getStr("view/keyboardInput/confirm"), brls::BUTTON_START, [this, onConfirm](brls::View*) mutable {
    ...
    // X：返回
    m_keyboard->registerAction(brls::getStr("view/keyboardInput/back"), brls::BUTTON_X, [this](brls::View*) {
        Audio::instance()->play(SoundEffect::Enter);
        closeInput();
        return true;
    });
...
void KeyboardInput::show(std::function<void(std::string)> onConfirm, std::string placeholder, int maxLength) {
    ...
    brls::Application::pushActivity(new brls::Activity(input), brls::TransitionAnimation::NONE);
}
