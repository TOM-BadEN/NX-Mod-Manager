...
void mtpDialog::open(std::vector<MtpMount> mounts) {
    ...
    // 初始状态：等待连接
    text->setText(brls::getStr("view/mtpDialog/waiting"));
    ...
    dialog->addButton(brls::getStr("view/mtpDialog/end"), endMtp);
    dialog->onB(endMtp);
    dialog->open();
        ...
        brls::sync([=] {
            ...
            switch (eventCopy.event) {
                case MtpEvent::Connected:
                    text->setMarginBottom(0);
                    text->setText(brls::getStr("view/mtpDialog/connected"));
                    break;
                ...
                case MtpEvent::ReadEnd: {
                    ...
                    state->idleDelayId = brls::delay(2000, [=] {
                        ...
                        text->setMarginBottom(0);
                        text->setText(brls::getStr("view/mtpDialog/completed", std::to_string(completed)));
                        progressArea->setVisibility(brls::Visibility::GONE);
                    });
                    break;
                }
