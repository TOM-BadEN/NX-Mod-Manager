...
// XML 布局在 resources/xml/view/dialog/customDialog.xml
...
CustomDialog::CustomDialog(brls::Box* contentView) {
    this->inflateFromXMLRes("xml/view/dialog/customDialog.xml");
...
void CustomDialog::rebuildButtons() {
    ...
    if (this->buttons.size() > 0) {
        ...
        button1->registerAction("", brls::BUTTON_A, [cb0](brls::View*) {
    ...
    if (this->buttons.size() > 1) {
        ...
        button2->registerAction("", brls::BUTTON_A, [cb1](brls::View*) {
            Audio::instance()->play(SoundEffect::Enter);
            cb1();
            return true;
        }, true, false, brls::SOUND_CLICK);
    ...
    if (this->buttons.size() > 2) {
        ...
        button3->registerAction("", brls::BUTTON_A, [cb2](brls::View*) {
            Audio::instance()->play(SoundEffect::Enter);
            cb2();
            return true;
        }, true, false, brls::SOUND_CLICK);
...
void CustomDialog::registerBAction() {
    ...
    appletFrame->registerAction("", brls::BUTTON_B, [action](brls::View*) {
        Audio::instance()->play(SoundEffect::Enter);
        action();
        return true;
    }, true);
...
void CustomDialog::show(std::string text, std::vector<ButtonConfig> buttons, std::function<void()> bAction) {
