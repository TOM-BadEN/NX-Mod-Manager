...
brls::Label* s_title = nullptr;          // 标题文本
brls::Label* s_left = nullptr;           // 左侧进度文本
brls::Label* s_right = nullptr;          // 右侧进度文本
brls::Rectangle* s_mainFill = nullptr;   // 主进度条填充区域
brls::Label* s_percentage = nullptr;     // 主进度百分比文本
brls::Rectangle* s_subFill = nullptr;    // 副进度条填充区域

} // namespace

namespace ProgressDialog {

void show(const std::string& title, std::vector<CustomDialog::ButtonConfig> buttons, std::function<void()> bAction)
{
    auto* content = dynamic_cast<brls::Box*>(brls::View::createFromXMLResource("view/dialog/progressDialog.xml"));
    if (!content) return;
