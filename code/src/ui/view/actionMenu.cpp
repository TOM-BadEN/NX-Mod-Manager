...
#include "ui/view/actionMenuItem.hpp"
...
private:
    ActionMenu* m_menu;      // 所属菜单
    MenuPageConfig* m_page;  // 当前菜单页配置
};
...
void ActionMenu::draw(NVGcontext* vg, float x, float y, float width, float height, brls::Style style, brls::FrameContext* ctx) {
    if (m_prevFocus && brls::Application::getInputType() != brls::InputType::TOUCH) {
        ...
        if (m_clipView) {
            nvgSave(vg);
            nvgIntersectScissor(vg, m_clipView->getX(), m_clipView->getY(), m_clipView->getWidth(), m_clipView->getHeight());
        }
        ...
        NVGpaint shadowPaint = nvgBoxGradient(vg, fx, fy + style["brls/highlight/shadow_width"], fw, fh, cr * 2, style["brls/highlight/shadow_feather"], nvgRGBA(0, 0, 0, (int)style["brls/highlight/shadow_opacity"]), brls::TRANSPARENT);
        nvgBeginPath(vg);
        nvgRect(vg, fx - shadowOffset, fy - shadowOffset, fw + shadowOffset * 2, fh + shadowOffset * 3);
        ...
        NVGcolor pulsationColor = nvgRGBAf(colorAnim * color1.r + (1 - colorAnim) * color1.r, colorAnim * color1.g + (1 - colorAnim) * color1.g, colorAnim * color1.b + (1 - colorAnim) * color1.b, 1.0f);
        ...
        borderColor.a = 0.5f;
        NVGpaint border1 = nvgRadialGradient(vg, fx + gradientX * fw, fy + gradientY * fh, sw * 10, sw * 40, borderColor, brls::TRANSPARENT);
        nvgBeginPath(vg);
        ...
        // 光点 2
        NVGpaint border2 = nvgRadialGradient(vg, fx + (1 - gradientX) * fw, fy + (1 - gradientY) * fh, sw * 10, sw * 40, borderColor, brls::TRANSPARENT);
        nvgBeginPath(vg);
...
void ActionMenu::updateMultiSelectActions(bool multiSelect) {
    ...
    // 多选模式：注册 + 键提交
    m_actionStart = registerAction(brls::getStr("view/actionMenu/submit"), brls::ControllerButton::BUTTON_START, [this](brls::View*) {
...
void ActionMenu::startAsync(size_t index, const MenuItemConfig& item) {
    ...
    auto asyncAction = item.asyncAction;

    // 提交任务到线程池
    ThreadPool::instance().submit([this, asyncAction](std::stop_token token) {
        auto result = asyncAction(token);
...
void ActionMenu::animLoop() {
    ...
    brls::sync([this, token] {
        ...
        if (++m_animFrame % 10 == 0) {
            ...
            if (cell) static_cast<ActionMenuItem*>(cell)->setItem(m_menuStack.back().page->items[m_asyncIndex].getTitle(), frames[(m_animFrame / 10) % std::size(frames)]);
        }
...
void ActionMenu::stopAsync() {
    ...
    if (cell) {
        static_cast<ActionMenuItem*>(cell)->setItem(items[m_asyncIndex].getTitle(), items[m_asyncIndex].getBadge());
    }
