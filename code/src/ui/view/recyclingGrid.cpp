...
brls::View* RecyclingGrid::getNextCellFocus(brls::FocusDirection direction, brls::View* currentView) {
    ...
    if (m_contentBox->getAxis() == brls::Axis::ROW &&
        ...
        if (row_currentFocus) {
            if (!row_currentFocus->getFrame().inscribed(getFrame())) {
                float localY = row_currentFocus->getLocalY();
                brls::View* p = row_currentFocus->getParent();
                while (p && p != m_contentBox) { localY += p->getLocalY(); p = p->getParent(); }
                float targetOffset;
                if (direction == brls::FocusDirection::DOWN) targetOffset = localY + row_currentFocus->getHeight() - getHeight();
                else targetOffset = localY;
                float contentHeight = m_contentBox->getHeight();
                float bottomLimit = contentHeight - getHeight();
                if (targetOffset < 0) targetOffset = 0;
                if (targetOffset > bottomLimit) targetOffset = bottomLimit;
                setContentOffsetY(targetOffset, false);
            }
            itemsRecyclingLoop();
