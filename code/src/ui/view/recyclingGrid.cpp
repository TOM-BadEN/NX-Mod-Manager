...
RecyclingGridItem::RecyclingGridItem() {
    setFocusable(true);
    registerAction("", brls::BUTTON_A, [this](View* view) {
        auto* parent = getParent();
        auto* grandParent = parent ? parent->getParent() : nullptr;
        auto* recycler = dynamic_cast<RecyclingGrid*>(grandParent);
        if (recycler && recycler->isInteractionLocked()) {
            return true;
        }
        Audio::instance()->play(SoundEffect::Enter);
        if (recycler && canTriggerSelection()) recycler->getDataSource()->onItemSelected(recycler, index);
        return true;
...
void RecyclingGridItem::onFocusGained() {
    auto* parent = getParent();
    auto* grandParent = parent ? parent->getParent() : nullptr;
    auto* recycler = dynamic_cast<RecyclingGrid*>(grandParent);
    bool locked = recycler && recycler->isInteractionLocked();
    setHideHighlight(false);
    setHideClickAnimation(locked);
    if (locked) {
        resetClickAnimation();
    }
    Box::onFocusGained();
...
void RecyclingGridItem::onFocusLost() {
...
SkeletonCell::SkeletonCell() {
    setCornerRadius(6);
    setHighlightCornerRadius(6);
}

RecyclingGridItem* SkeletonCell::create() { return new SkeletonCell(); }

void SkeletonCell::draw(NVGcontext* vg, float x, float y, float width, float height, brls::Style style, brls::FrameContext* ctx) {
    if (m_fast) return; // 快速模式：完全透明，不绘制任何内容
...
brls::View* RecyclingGridContentBox::getNextFocus(brls::FocusDirection direction, brls::View* currentView) {
    if (m_recycler->isInteractionLocked()) {
        return currentView;
    }
...
RecyclingGrid::RecyclingGrid() {
    ...
    registerPercentageXMLAttribute("paddingRight", [this](float percentage) { setPaddingRightPercentage(percentage); });

    registerPercentageXMLAttribute("paddingLeft", [this](float percentage) { setPaddingLeftPercentage(percentage); });

    registerCell("Skeleton", SkeletonCell::create);

    // LB/RB 翻页（hidden=true：不在底部按钮栏显示）
    registerAction("", brls::ControllerButton::BUTTON_LB, [this](...) {
        if (!m_interactionLocked) {
            flipScreen(-1);
        }
        return true;
    }, true, true);
    registerAction("", brls::ControllerButton::BUTTON_RB, [this](...) {
        if (!m_interactionLocked) {
            flipScreen(1);
        }
        return true;
...
void RecyclingGrid::draw(NVGcontext* vg, float x, float y, float width, float height, brls::Style style, brls::FrameContext* ctx) {
    if (m_deferredFocusIndex >= 0) {
        int idx = m_deferredFocusIndex;
        m_deferredFocusIndex = -1;
        setDefaultCellFocus(idx);
        reloadData();
        instantFocus(idx);
...
void RecyclingGrid::reloadData() {
    ...
    if (!isFlowMode || spanCount != 1) {
        m_contentBox->setHeight((estimatedRowHeight + estimatedRowSpace) * (float)getRowCount() - estimatedRowSpace + m_paddingTop + m_paddingBottom);
...
void RecyclingGrid::reloadItem(size_t index) {
    if (!m_layouted || !m_dataSource || index >= m_dataSource->getItemCount()) return;

    auto& children = m_contentBox->getChildren();
    auto iterator  = std::find_if(children.begin(), children.end(), [index](brls::View* view) {
        return static_cast<RecyclingGridItem*>(view)->getIndex() == index;
    });
    if (iterator == children.end()) return;

    auto* oldCell       = static_cast<RecyclingGridItem*>(*iterator);
    bool focused        = brls::Application::getCurrentFocus() == oldCell;
    bool lastFocused    = m_contentBox->getLastFocusedView() == oldCell;
    size_t cellPosition = std::distance(children.begin(), iterator);

    queueReusableCell(oldCell);
    oldCell->willDisappear(true);
    children.erase(iterator);

    RecyclingGridItem* cell = m_dataSource->cellForRow(this, index);
    layoutCellAt(cell, index);
    children.insert(children.begin() + cellPosition, cell);
    ...
    *userdata        = index;
    ...
    // 快速骨架模式：第一个真实卡片出现时自动解除交互锁定
    if (m_fastSkeletonMode && m_interactionLocked &&
        oldCell->reuseIdentifier == "Skeleton" && cell->reuseIdentifier != "Skeleton") {
        setInteractionLocked(false);
    }

    if (lastFocused) m_contentBox->setLastFocusedView(cell);
    if (focused) {
        instantFocus(index);
        if (oldCell != cell) oldCell->setParent(nullptr);
    }
}

void RecyclingGrid::deferReload(int focusIndex) {
    m_deferredFocusIndex = focusIndex;
}
...
void RecyclingGrid::notifyDataChanged() {
    ...
    if (m_dataSource) {
        if (isFlowMode) {
            ...
            m_contentBox->setHeight((estimatedRowHeight + estimatedRowSpace) * getRowCount() - estimatedRowSpace + m_paddingTop + m_paddingBottom);
        }
    }
    m_requestNextPage = false;
}
...
void RecyclingGrid::setInteractionLocked(bool locked) {
    m_interactionLocked = locked;

    for (auto* recognizer : getGestureRecognizers()) {
        recognizer->setEnabled(!locked);
    }

    for (auto* item : getGridItems()) {
        applyInteractionLockedToItem(item);
    }
}

bool RecyclingGrid::isInteractionLocked() const { return m_interactionLocked; }

void RecyclingGrid::setFastSkeletonMode(bool fast) {
    if (m_fastSkeletonMode == fast) return;
    m_fastSkeletonMode = fast;
    setInteractionLocked(fast); // 开启时锁定交互，关闭时解除
}

void RecyclingGrid::applyInteractionLockedToItem(RecyclingGridItem* item) {
    item->setHideClickAnimation(m_interactionLocked);
    if (m_interactionLocked) item->resetClickAnimation();

    for (auto* recognizer : item->getGestureRecognizers()) {
        recognizer->setEnabled(!m_interactionLocked);
    }
}
...
float RecyclingGrid::layoutCellAt(RecyclingGridItem* cell, size_t index) {
    ...
    applyInteractionLockedToItem(cell);
    cell->setDetachedPositionX(cellX);
    cell->setDetachedPositionY(getHeightByCellIndex(index) + m_paddingTop);
    cell->setIndex(index);

    // 骨架样式跟随快速模式：开启时透明且仅索引 0 可聚焦，关闭时恢复渐变且全部可聚焦
    if (auto* skeleton = dynamic_cast<SkeletonCell*>(cell)) {
        skeleton->setFastMode(m_fastSkeletonMode);
        skeleton->setFocusable(!m_fastSkeletonMode || index == 0);
    }
...
void RecyclingGrid::addCellAt(size_t index, bool downSide) {
    RecyclingGridItem* cell = m_dataSource->cellForRow(this, index);
    float cellHeight        = layoutCellAt(cell, index);
...
void RecyclingGrid::queueReusableCell(RecyclingGridItem* cell) {
    ...
    // 断开 parent 链，页面返回时走 fallback 恢复焦点；
    // 但焦点 cell 保留 parent 链，避免触摸拖动后导航失效
    if (brls::Application::getCurrentFocus() != cell) cell->setParent(nullptr);
    cell->cacheForReuse();
}
...
void RecyclingGrid::itemsRecyclingLoop() {
    ...
    if (!m_interactionLocked && visibleMax + 1 >= getItemCount()) {
