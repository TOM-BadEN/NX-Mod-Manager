...
KeyboardButton::KeyboardButton() {
    ...
    setCornerRadius(4);
    setBackgroundColor(brls::Application::getTheme().getColor("app/cardBg"));
    setShadowType(brls::ShadowType::NONE);
}

void KeyboardButton::draw(NVGcontext* vg, float x, float y, float width, float height, brls::Style style, brls::FrameContext* ctx) {
    auto drawShadow = [=](float offset, float feather, float opacity) {
        NVGcolor inner = ctx->theme["app/keyboardKeyDepth"];
        NVGcolor outer = inner;
        inner.a *= opacity;
        outer.a = 0.0f;

        NVGpaint shadow = nvgBoxGradient(vg, x, y + offset, width, height, 4, feather, inner, outer);
        nvgBeginPath(vg);
        nvgRect(vg, x - feather, y - feather, width + feather * 2, height + feather * 2 + offset);
    ...
    drawShadow(1, 7, 0.35f);
    drawShadow(2, 3, 0.75f);
...
void Keyboard::buildLayout() {
    ...
    const float inputSpacing = 26.0f;
    ...
    m_inputLabel->setMarginTop(inputSpacing);
    m_inputLabel->setMarginBottom(inputSpacing);
    ...
    m_spaceButton = new KeyboardButton();
    ...
    m_deleteButton = new KeyboardButton();
