...
void AppShell::shakeHeaderNav(bool right) {
    m_header->shakeHeaderNav(right);
}
...
void AppShell::applyShellState(const ShellStateData& state) {
    m_header->setHeaderState(state.headerState);
    m_footer->setIndexText(state.indexText);
    m_footer->setBackgroundTheme(state.footerBackgroundTheme);
}
