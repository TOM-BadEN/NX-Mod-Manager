...
void StoreGameList::submitNextIcons(int count) {
    ...
    for (int i = 0; i < toSubmit; i++) {
        ...
        std::string url = api::url::game::icon(list[gameIdx].gameTid);
        list[gameIdx].iconId = -2;  // 标记为下载中
        m_pendingIcons++;

        ThreadPool::instance().submit([this, url, gameIdx](std::stop_token token) {
            auto response = http::downloadImage(url, token);
            if (token.stop_requested()) return;

            brls::sync([this, gameIdx, response = std::move(response), token]() mutable {
                if (token.stop_requested()) return;
                onIconDownloaded(gameIdx, std::move(response));
            });
        }, token);
    }
}
...
void StoreGameList::onIconDownloaded(int gameIdx, http::Response response) {
    m_pendingIcons--;

    if (!response.ok() || response.body.empty()) {
        m_manager.storeGameList()[gameIdx].iconId = -1;
    } else {
        auto img = webpDecoder::decode(response.body.data(), response.body.size());
