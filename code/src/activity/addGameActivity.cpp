...
void AddGameActivity::onGameCardClicked(size_t index) {
    ...
    if (mods.empty()) {
        ScrollDialog::show(
            createTransitEmptyBox(),
            brls::getStr("addGameActivity/cancel"), [] { ScrollDialog::close(); },
            brls::getStr("addGameActivity/confirm"), [] { ScrollDialog::close(); },
            [] { ScrollDialog::close(); });
        return;
    }
...
brls::Box* AddGameActivity::createTransitEmptyBox() {
    LongTextBoxConfig content;

    auto& method = content.addEntry();
    method.addTitle(brls::getStr("addGameActivity/transitEmptyTitle"));
    method.addBody(brls::getStr("addGameActivity/firstLaunchMethodBody"));

    auto& format = content.addEntry();
    format.addTitle(brls::getStr("addGameActivity/transitEmptyFormatTitle"));
    format.addBody(brls::getStr("addGameActivity/firstLaunchFormatBody"));

    auto& notice = content.addEntry();
    notice.addTitle(brls::getStr("addGameActivity/transitEmptyNoticeTitle"));
    notice.addBody(brls::getStr("addGameActivity/firstLaunchFilenameBody"));
...
void AddGameActivity::setNacpActionsAvailable(bool available) {
    m_frame->setActionAvailable(brls::BUTTON_BACK, available);
    m_frame->setActionAvailable(brls::BUTTON_Y, available);
}
...
brls::Box* AddGameActivity::createFirstLaunchBox() {
    LongTextBoxConfig content;

    auto& notice = content.addEntry();
    notice.addTitle(brls::getStr("addGameActivity/firstLaunchNoticeTitle"));
    notice.addBody(brls::getStr("addGameActivity/firstLaunchNoticeBody"));

    auto& method = content.addEntry();
    method.addTitle(brls::getStr("addGameActivity/firstLaunchMethodTitle"));
    method.addBody(brls::getStr("addGameActivity/firstLaunchMethodBody"));

    auto& format = content.addEntry();
    format.addTitle(brls::getStr("addGameActivity/firstLaunchFormatTitle"));
    format.addBody(brls::getStr("addGameActivity/firstLaunchFormatBody"));

    auto& filename = content.addEntry();
    filename.addTitle(brls::getStr("addGameActivity/firstLaunchFilenameTitle"));
    filename.addBody(brls::getStr("addGameActivity/firstLaunchFilenameBody"));
...
void AddGameActivity::runFirstLaunchDialog() {
    ...
    brls::sync([this]() {
        ...
        LongPressDialog::show(createFirstLaunchBox(), brls::getStr("addGameActivity/firstLaunchBtn"), 5.0f, onConfirm);
    });
}
