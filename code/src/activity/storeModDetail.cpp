...
StoreModDetail::StoreModDetail(int modId, std::string gameTid, std::string gameName, GameManager& gameManager, ReturnCallback onReturn, ModManager* localModManager, std::string gameVersion, bool downloaded)
    : m_manager(modId, std::move(gameTid), downloaded), m_gameName(std::move(gameName)), m_gameManager(gameManager), m_onReturn(std::move(onReturn)), m_localModManager(localModManager), m_gameVersion(std::move(gameVersion)) {}
...
void StoreModDetail::loadScreenshots() {
    auto url1 = api::url::mod::screenshot(m_manager.gameTid(), m_manager.modId(), 1);
    auto url2 = api::url::mod::screenshot(m_manager.gameTid(), m_manager.modId(), 2);
    auto token = m_stopSource.get_token();
    ThreadPool::instance().submit([this, url1, url2](std::stop_token token) {
        // 并行下载两张截图
        auto future1 = std::async(std::launch::async, [&url1, &token] {
            return http::get(url1, token);
        });
        auto future2 = std::async(std::launch::async, [&url2, &token] {
            return http::get(url2, token);
        });
        ...
        brls::sync([this, body1 = std::move(body1), body2 = std::move(body2), ok1, ok2, token] {
            if (token.stop_requested()) return;
...
void StoreModDetail::onDownloadAction() {
    ...
    auto onConfirm = [this] {
        ...
        ThreadPool::instance().submit([this, modId, gameTid, gameName, modName](std::stop_token token) {
            // ── 阶段一：获取下载信息 ──
            ...
            brls::sync([this, gameTid, gameNameEn, gameName, modDirName, tempPath, modName, token] {
                if (token.stop_requested()) return;
                ...
                std::string backKey = m_localModManager ? "storeModDetail/backToModList" : "storeModDetail/backToHome";
                bool hasLocal = m_localModManager != nullptr;
                CustomDialog::show(brls::getStr("storeModDetail/downloadComplete", modName), {
                    {brls::getStr("storeModDetail/continueBrowse"), [] { CustomDialog::close(); }},
                    {brls::getStr(backKey), [hasLocal] {
                        int popCount = static_cast<int>(brls::Application::getActivitiesStack().size()) - (hasLocal ? 3 : 2);
