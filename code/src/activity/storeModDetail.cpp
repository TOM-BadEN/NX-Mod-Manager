...
std::string StoreModDetail::commentSubmitNickname() const {
    std::string nickname = Settings::getString("modShop", "nickname");
    return nickname.empty() ? ANONYMOUS_NICKNAME : nickname;
}
...
void StoreModDetail::showChangelogDialog() {
    ...
    ScrollDialog::show(text, brls::getStr("storeModDetail/cancel"), [] { ScrollDialog::close(); }, brls::getStr("storeModDetail/confirm"), [] { ScrollDialog::close(); });
}
...
void StoreModDetail::onCommentAction() {
    // 获取昵称
    std::string nickname = commentSubmitNickname();
