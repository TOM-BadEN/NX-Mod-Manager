...
StoreModDetail::StoreModDetail(int modId, std::string gameTid, std::string gameName, GameManager& gameManager, ReturnCallback onReturn, ModManager* localModManager, std::string gameVersion)
    : m_manager(modId, std::move(gameTid)), m_gameName(std::move(gameName)), m_gameManager(gameManager), m_onReturn(std::move(onReturn)), m_localModManager(localModManager), m_gameVersion(std::move(gameVersion)) {}
