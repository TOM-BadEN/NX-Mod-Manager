...
void Home::onContentAvailable() {
    ...
    m_frame->setActionAvailable(brls::BUTTON_START, false);
...
void Home::setupSettingsMenu() {
    ...
    auto& soundItem = m_settingsMenu.addItem(brls::getStr("home/soundEffect"), brls::getStr("home/soundEffectDesc"));
    ...
    auto& transitItem = m_settingsMenu.addItem(brls::getStr("home/clearTransitItem"), brls::getStr("home/clearTransitDesc"));
    ...
    auto& resetItem = m_settingsMenu.addItem(brls::getStr("home/resetState"), brls::getStr("home/resetStateDesc"));
...
void Home::setupAddProjectMenu() {
    m_addProjectMenu.title = "新增项目";

    auto& storeAddItem = m_addProjectMenu.addItem("商店添加", "从在线模组商店挑选游戏，在主页使用 \uE0E6 快速跳转到商店\n - 游戏商店\n - 模组商店\n - 上传模组\n - 用户昵称");
    storeAddItem.setBadge("\uE14A");
    storeAddItem.setSubmenu(&m_storeAddMenu);

    auto& localAddItem = m_addProjectMenu.addItem("本地添加", brls::getStr("home/addGameDesc"));
    localAddItem.setBadge("\uE14A");
    localAddItem.setAction([this]{ openAddGamePage(); });
}

void Home::setupStoreAddMenu() {
    m_storeAddMenu.title = "商店添加";
    m_storeAddMenu.shouldShowFakeHighlight = [this]{ return !m_gameManager.games().empty(); };

    auto& gameStoreItem = m_storeAddMenu.addItem("游戏商店", "浏览在线商店中的游戏列表");
    gameStoreItem.setBadge("\uE14A");
    gameStoreItem.setAction([this] {
        pageNav::push(new StoreGameList(m_gameManager), pageNav::Anim::SLIDE_LEFT);
    });

    auto& modStoreItem = m_storeAddMenu.addItem("模组商店", "浏览当前游戏的在线模组列表");
    ...
    auto& nicknameItem = m_storeAddMenu.addItem(brls::getStr("home/nickname"), brls::getStr("home/nicknameDesc"));
...
void Home::setupMenu() {
    ...
    auto& addProjectItem = m_menu.addItem("新增项目", "选择对应方法，添加游戏项目到主页\n - 商店添加\n - 本地添加");
    ...
    m_nacpLoader = util::async([this](std::stop_token token) {
        ...
        if (!token.stop_requested()) {
            brls::sync([this]() {
                ...
                m_frame->setActionAvailable(brls::BUTTON_START, true);
                m_nacpComplete = true;
                if (m_onNacpComplete) {
                    m_onNacpComplete();
                    m_onNacpComplete = nullptr;
