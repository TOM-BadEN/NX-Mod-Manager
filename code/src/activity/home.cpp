...
void Home::setupGridPage() {
    ...
    auto* ds = new GameCardDS(
        m_gameManager.games(),
        [this](size_t index) { pageNav::push(new ModList(index, m_gameManager)); },
        [this](uint64_t appId) { launchGame(appId); }
    );
...
void Home::launchGame(uint64_t appId) {
    ...
    CustomDialog::show(brls::getStr("home/launchFailed", format::resultHex(rc)), {
