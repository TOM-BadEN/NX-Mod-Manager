...
void Home::onContentAvailable() {
    ...
    m_frame->registerAction(brls::getStr("home/sortAsc"), brls::BUTTON_Y, [this](...) {
    ...
    // + 键：打开商店菜单
    m_frame->registerAction(brls::getStr("home/store"), brls::BUTTON_START, [this](...) {
        Audio::instance()->play(SoundEffect::Enter);
        m_storeMenu.show();
        return true;
    });
...
void Home::onResume() {
    ...
    if (appId == 0) return;
    m_gameManager.sort(m_sortAsc);
    int newIdx = m_gameManager.findByAppId(appId);
    // 空主页添加游戏后，需要从空提示切回网格，否则 deferReload 无法触发（GONE 不会 draw）
...
void Home::toggleSort() {
    ...
    m_frame->updateActionHint(brls::BUTTON_Y, m_sortAsc ? brls::getStr("home/sortAsc") : brls::getStr("home/sortDesc"));
...
void Home::openAddGamePage() {
    if (m_nacpComplete) {
        pageNav::push(new AddGameActivity(m_gameManager));
        return;
    }
...
void Home::resetState() {
    ...
    LongPressDialog::show(brls::getStr("home/resetStateNotice"), brls::getStr("home/resetStateBtn"), 3.0f, onConfirm, [] { LongPressDialog::close(); });
}
...
void Home::setupGameNameMenu() {
    ...
    onlineFetchNameItem.setDisabled([this] { return m_gameManager.games().empty() || !deviceInfo::isNetworkAvailable(); });
    onlineFetchNameItem.setBadge([] { return deviceInfo::isNetworkAvailable() ? "" : brls::getStr("home/noNetwork"); });
...
void Home::setupGameManageMenu() {
    ...
    auto& addGameItem = m_gameManageMenu.addItem(brls::getStr("home/addGame"), brls::getStr("home/addGameDesc"));
    addGameItem.setAction([this]{ openAddGamePage(); });
    ...
    auto& transitItem = m_gameManageMenu.addItem(brls::getStr("home/clearTransitItem"), brls::getStr("home/clearTransitDesc"));
...
void Home::setupSettingsMenu() {
    ...
    auto& langItem = m_settingsMenu.addItem(brls::getStr("home/language"), brls::getStr("home/languageDesc"));
    langItem.setBadge([]{
    ...
    auto& themeItem = m_settingsMenu.addItem(brls::getStr("home/themeColor"), brls::getStr("home/themeColorDesc"));
    themeItem.setBadge([]{
    ...
    auto& fpsItem = m_settingsMenu.addItem(brls::getStr("home/fpsMonitor"), brls::getStr("home/fpsMonitorDesc"));
    ...
    auto& memItem = m_settingsMenu.addItem(brls::getStr("home/memMonitor"), brls::getStr("home/memMonitorDesc"));
...
void Home::setupStoreMenu() {
    m_storeMenu.title = brls::getStr("home/storeMenuTitle");
    m_storeMenu.shouldShowFakeHighlight = [this]{ return !m_gameManager.games().empty(); };

    auto& gameListItem = m_storeMenu.addItem(brls::getStr("home/storeGameList"), brls::getStr("home/storeGameListDesc"));
    gameListItem.setBadge("\uE14A");
    gameListItem.setAction([this] {
        pageNav::push(new StoreGameList(m_gameManager));
    });

    auto& modListItem = m_storeMenu.addItem(brls::getStr("home/storeModList"), brls::getStr("home/storeModListDesc"));
    modListItem.setDisabled([this] { return m_gameManager.games().empty(); });
    ...
    modListItem.setAction([this] {
        ...
        pageNav::push(new StoreModList(tid, game.displayName, m_gameManager));
    });

    auto& uploadItem = m_storeMenu.addItem(brls::getStr("home/uploadMod"), brls::getStr("home/uploadModDesc"));
    uploadItem.setBadge("\uE14A");
    uploadItem.setAction([] {
        QrCodeView::show(config::websiteUrl);
    });

    auto& nicknameItem = m_storeMenu.addItem(brls::getStr("home/nickname"), brls::getStr("home/nicknameDesc"));
...
void Home::setupMenu() {
    ...
    auto& favoriteItem = m_menu.addItem(brls::getStr("home/favorite"), brls::getStr("home/favoriteDesc"));
    favoriteItem.setDisabled([this]{ return m_gameManager.games().empty(); });
    favoriteItem.setTitle([this]{
        if (m_gameManager.games().empty()) return brls::getStr("home/favorite");
        return m_gameManager.games()[m_focusedIndex.load()].isFavorite ? brls::getStr("home/unfavorite") : brls::getStr("home/favorite");
    });
    favoriteItem.setAction([this]{ toggleFavorite(); });

    auto& renameSubmenuItem = m_menu.addItem(brls::getStr("home/renameSubmenu"), brls::getStr("home/renameSubmenuDesc"));
    renameSubmenuItem.setDisabled([this]{ return m_gameManager.games().empty(); });
    renameSubmenuItem.setBadge("\uE14A");
    renameSubmenuItem.setSubmenu(&m_gameNameMenu);
    ...
    updateItem.setDisabled([]{ return !deviceInfo::isNetworkAvailable(); });
    updateItem.setBadge([]() -> std::string {
        if (!deviceInfo::isNetworkAvailable()) return brls::getStr("home/noNetwork");
    ...
    updateItem.setAction([this](std::any result) { showUpdateResult(result); });
...
std::any Home::checkForUpdate(std::stop_token token) {
    ...
    updater.checkSync(token, APP_VERSION);
    return updater.hasUpdate();
}

void Home::showUpdateResult(std::any result) {
    auto& updater = AppUpdater::instance();
    ...
    ScrollDialog::show(
        brls::getStr("home/updateDetail", updater.tagName(), updater.publishTime(), updater.releaseNotes()),
...
void Home::startUpdateDownload() {
    auto onCancel = [this] { m_updateTask.request_stop(); };

    ProgressDialog::show(brls::getStr("home/downloading"), {{brls::getStr("home/cancel"), onCancel}}, onCancel);
    ...
    m_updateTask = util::async([](std::stop_token token) {
        ...
        auto resp = AppUpdater::instance().download(token, onProgress);
        bool cancelled = token.stop_requested();

        brls::sync([resp, cancelled] {
            if (cancelled) {
                CustomDialog::close();
                return;
            }
            if (!resp.ok()) {
                std::string msg;
                if (resp.code == 429) msg = brls::getStr("home/downloadRateLimit");
                else msg = brls::getStr("home/downloadFailed");
                CustomDialog::show(msg, {{brls::getStr("home/ok"), []{ CustomDialog::close(); }}});
                return;
            }
            CustomDialog::show(brls::getStr("home/restarting"), {}, []{});
...
bool Home::checkDuplicates() {
    int dupCount = m_gameManager.duplicateCount();
    if (dupCount == 0) return false;
    std::string msg = brls::getStr("home/duplicateWarning", dupCount);
    CustomDialog::show(msg, {{brls::getStr("home/ok"), [] { CustomDialog::close(); }}});
    return true;
}
...
void Home::runStartupDialogs() {
    ...
    bool isFirstLaunch = Settings::getBool("app", "firstLaunch", true);
    if (!isFirstLaunch) {
        brls::sync([this]() { checkDuplicates(); });
        return;
    }
    brls::sync([this]() {
        auto onConfirm = [this]() {
            Settings::setBool("app", "firstLaunch", false);
            if (!checkDuplicates()) LongPressDialog::close();
        };
        LongPressDialog::show(brls::getStr("home/firstLaunchNotice"), brls::getStr("home/firstLaunchBtn"), 5.0f, onConfirm);
    });
}
