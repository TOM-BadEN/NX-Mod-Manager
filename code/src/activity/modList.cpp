...
void ModList::submitUpdateCheck() {
    if (!deviceInfo::isNetworkAvailable()) return;
...
bool ModList::showMHRiseFirstLaunchDialog() {
    if (m_modManager.modGameType() != ModGameType::MHRise) return false;
    if (!Settings::getBool("modList", "mhriseFirstLaunch", true)) return false;

    auto onConfirm = []() {
        Settings::setBool("modList", "mhriseFirstLaunch", false);
        LongPressDialog::close();
    };
    LongPressDialog::show(brls::getStr("modList/mhriseFirstLaunchNotice"), brls::getStr("modList/firstLaunchBtn"), 5.0f, onConfirm);
    return true;
}
...
void ModList::runFirstLaunchDialog() {
    ...
    brls::sync([this]() {
        auto onConfirm = [this]() {
            ...
            if (!showMHRiseFirstLaunchDialog()) LongPressDialog::close();
        };
