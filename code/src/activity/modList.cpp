...
void ModList::onContentAvailable() {
    ...
    m_frame->registerAction(brls::getStr("modList/zlStore"), brls::BUTTON_LT, [this](...) {
        ...
        pageNav::push(new StoreModList(tid, game.displayName, m_gameManager, &m_modManager, game.version, true), pageNav::Anim::SLIDE_LEFT);
        return true;
    });
...
bool ModList::checkMHRiseRules() {
    ...
    if (result == ModInstaller::mhrise::InstallPrecheckResult::GameNotInstalled) {
        msg = brls::getStr("modList/mhriseGameNotInstalled");
    } else if (result == ModInstaller::mhrise::InstallPrecheckResult::VersionChanged) {
        msg = brls::getStr("modList/mhriseVersionChanged");
    } else {
        msg = brls::getStr("modList/mhriseVersionUnsupported");
    }

    CustomDialog::show(msg, {
        {brls::getStr("modList/ok"), [] { CustomDialog::close(); }},
    });
    return false;
}
...
bool ModList::checkDontStarveRules(int index) {
    ...
    CustomDialog::show(brls::getStr("modList/dontStarveFrameworkMissing"), {
        {brls::getStr("modList/cancel"), [] { CustomDialog::close(); }},
        {brls::getStr("modList/confirm"), onContinue},
    });
    return false;
}
...
void ModList::setupViewModMenu() {
    ...
    modDetailItem.setAction([this] {
        ...
        auto& game = m_gameManager.games()[m_gameIndex];
        std::string tid = format::appIdHex(game.appId);
        pageNav::push(new StoreModDetail(mod.modID, tid, game.displayName, m_gameManager, nullptr, &m_modManager, game.version, true), pageNav::Anim::SLIDE_LEFT);
    });
...
brls::Box* ModList::createFirstLaunchBox() {
    ...
    basic.addTitle(brls::getStr("modList/firstLaunchBasicTitle"));
    basic.addBody(brls::getStr("modList/firstLaunchBasicBody"));

    auto& shortcuts = content.addEntry();
    shortcuts.addTitle(brls::getStr("modList/firstLaunchShortcutsTitle"));
    shortcuts.addBody(brls::getStr("modList/firstLaunchShortcutsBody"), 1.3f);

    auto& localAdd = content.addEntry();
    localAdd.addTitle(brls::getStr("modList/firstLaunchLocalAddTitle"));
    localAdd.addBody(brls::getStr("modList/firstLaunchLocalAddBody"));

    auto& notice = content.addEntry();
    notice.addTitle(brls::getStr("modList/firstLaunchNoticeTitle"));
    notice.addBody(brls::getStr("modList/firstLaunchNoticeBody"));
...
brls::Box* ModList::createMHRiseFirstLaunchBox() {
    ...
    auto& notice = content.addEntry();
    notice.addTitle(brls::getStr("modList/mhriseFirstLaunchTitle"));
    notice.addBody(brls::getStr("modList/mhriseFirstLaunchBody"));

    auto& troubleshooting = content.addEntry();
    troubleshooting.addTitle(brls::getStr("modList/mhriseFirstLaunchTroubleshootingTitle"));
    troubleshooting.addBody(brls::getStr("modList/mhriseFirstLaunchTroubleshootingBody"));
...
brls::Box* ModList::createDontStarveFirstLaunchBox() {
    ...
    auto& notice = content.addEntry();
    notice.addTitle(brls::getStr("modList/dontStarveFirstLaunchTitle"));
    notice.addBody(brls::getStr("modList/dontStarveFirstLaunchBody"));

    auto& usage = content.addEntry();
    usage.addTitle(brls::getStr("modList/dontStarveFirstLaunchUsageTitle"));
    usage.addBody(brls::getStr("modList/dontStarveFirstLaunchUsageBody"));

    auto& troubleshooting = content.addEntry();
    troubleshooting.addTitle(brls::getStr("modList/dontStarveFirstLaunchTroubleshootingTitle"));
    troubleshooting.addBody(brls::getStr("modList/dontStarveFirstLaunchTroubleshootingBody"));
...
brls::Box* ModList::createUnmanagedModBox() {
    ...
    detected.addTitle(brls::getStr("modList/unmanagedDetectedTitle"));
    detected.addBody(brls::getStr("modList/unmanagedDetectedBody"));

    auto& extraction = content.addEntry();
    extraction.addTitle(brls::getStr("modList/unmanagedExtractionTitle"));
    extraction.addBody(brls::getStr("modList/unmanagedExtractionBody"));

    auto& notice = content.addEntry();
    notice.addTitle(brls::getStr("modList/unmanagedNoticeTitle"));
    notice.addBody(brls::getStr("modList/unmanagedNoticeBody"));
...
void ModList::showUnmanagedModDialog() {
    ...
    auto onBack = [] {
        ScrollDialog::close([] { pageNav::pop(); });
    };

    ScrollDialog::show(
        createUnmanagedModBox(),
        brls::getStr("modList/back"), onBack,
        brls::getStr("modList/confirm"), [this] { startUnmanagedModExtraction(); },
        onBack);
}
...
void ModList::startUnmanagedModExtraction() {
    ...
    if (!result.success) {
        std::string message = result.rollbackFailed ? brls::getStr("modList/unmanagedRollbackFailed", result.errorPath) : brls::getStr("modList/unmanagedExtractFailed", result.errorPath);
        auto returnHome = [] { CustomDialog::close([] { pageNav::pop(); }); };
        CustomDialog::show(message, {{brls::getStr("modList/backToHome"), returnHome}}, [] {});
        return;
    }
...
void ModList::showMHRiseFirstLaunchDialog() {
    ...
    LongPressDialog::show(createMHRiseFirstLaunchBox(), brls::getStr("modList/firstLaunchBtn"), 5.0f, onConfirm);
}
...
void ModList::showDontStarveFirstLaunchDialog() {
    ...
    LongPressDialog::show(createDontStarveFirstLaunchBox(), brls::getStr("modList/firstLaunchBtn"), 5.0f, onConfirm);
}
...
void ModList::runFirstLaunchDialog() {
    ...
    brls::sync([this]() {
        ...
        LongPressDialog::show(createFirstLaunchBox(), brls::getStr("modList/firstLaunchBtn"), 5.0f, onConfirm);
    });
}
