...
bool ModList::checkBeforeModInstall() {
...
bool ModList::checkMHRiseRules() {
    ...
    bool gameInstalled = m_gameManager.isGameInstalled(game.appId);
...
void ModList::toggleModInstall(int index) {
    ...
    if (installing && !checkBeforeModInstall()) return;
...
void ModList::showMHRiseFirstLaunchDialog() {
    if (m_modManager.modGameType() != ModGameType::MHRise) return;
    if (!Settings::getBool("modList", "mhriseFirstLaunch", true)) return;
...
void ModList::runFirstLaunchDialog() {
    ...
    if (!isFirstLaunch) {
        brls::sync([this]() { showMHRiseFirstLaunchDialog(); });
        return;
    }
    brls::sync([this]() {
        auto onConfirm = [this]() {
