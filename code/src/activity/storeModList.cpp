...
void StoreModList::onContentAvailable() {
    ...
    // + 键：上传模组
    m_frame->registerAction(brls::getStr("storeModList/uploadMod"), brls::BUTTON_START, [](...) {
        Audio::instance()->play(SoundEffect::Enter);
...
void StoreModList::openDetail(size_t index) {
    ...
    pageNav::push(new StoreModDetail(mod.modId, m_manager.gameTid(), m_gameName, m_gameManager, onReturn, m_localModManager, m_gameVersion, m_fromModList));
}
