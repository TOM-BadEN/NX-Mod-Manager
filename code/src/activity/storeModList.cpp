...
StoreModList::StoreModList(std::string gameTid, std::string gameName, GameManager& gameManager, ModManager* localModManager)
    : m_manager(std::move(gameTid)), m_gameName(std::move(gameName)), m_gameManager(gameManager), m_localModManager(localModManager) {}
...
void StoreModList::loadNextPage() {
    m_loading = true;
    auto alive = m_alive;
    m_loadTask = util::async([this, alive](std::stop_token token) {
        ...
        brls::sync([this, alive, result = std::move(result)]() mutable {
            ...
            if (!m_grid->getDataSource()) {
                ...
                m_grid->setDataSource(new StoreModListDS(m_manager.storeModList(), [this](size_t index) {
                    ...
                    pageNav::push(new StoreModDetail(mod.modId, m_manager.gameTid(), m_gameName, m_gameManager,
                        [this, index](int likes, int dislikes, int downloads, bool installed) {
                            auto& mod = m_manager.storeModList()[index];
                            mod.likeCount = likes;
                            mod.dislikeCount = dislikes;
                            mod.downloadCount = downloads;
                            mod.isInstalled = installed;
                            ...
                            if (cell) static_cast<StoreModCard*>(cell)->setMod(mod.modName, mod.uploadTime, mod.modType,
                                mod.gameVersion, likes, dislikes, downloads, installed);
                        }, m_localModManager));
                }));
...
void StoreModList::loadVersionMenu() {
    m_versionTask = util::async([this](std::stop_token token) {
        auto result = m_manager.fetchGameVersions(token);
        if (token.stop_requested()) return;
        auto alive = m_alive;
        brls::sync([this, alive, result = std::move(result)]() mutable {
            if (!alive->load()) return;
