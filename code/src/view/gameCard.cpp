...
void GameCard::setGame(const std::string& name, const std::string& version, const std::string& modCount) {
    ...
    m_version->setText(brls::getStr("views/gameCard/version", version));
    m_modCount->setText(modCount.empty() ? brls::getStr("views/gameCard/notAdded") : brls::getStr("views/gameCard/count", modCount));
}
...
void GameCard::prepareForReuse() {
    ...
    m_name->setText("");
    m_name->setTextColor(brls::Application::getTheme()["brls/text"]);
    m_version->setText("");
    m_modCount->setText("");
}
