...
RecyclingGrid::RecyclingGrid() {
    ...
    registerAction(brls::getStr("views/recyclingGrid/pageUp"), brls::ControllerButton::BUTTON_LB, [this](...) {
        if (!m_interactionLocked) {
            flipScreen(-1);
        }
        return true;
    }, true, true);
    registerAction(brls::getStr("views/recyclingGrid/pageDown"), brls::ControllerButton::BUTTON_RB, [this](...) {
        if (!m_interactionLocked) {
            flipScreen(1);
        }
        return true;
...
void RecyclingGrid::queueReusableCell(RecyclingGridItem* cell) {
    ...
    // 断开 parent 链，popActivity 走 fallback 恢复焦点；
