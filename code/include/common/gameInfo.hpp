...
struct GameInfo {
    ...
    bool isInstalled = false;    // 游戏本体是否已安装在主机
    ...
    bool isPending = true;        // 是否等待 FrameQueue 显示
};

/** @brief 添加游戏页面使用的已安装游戏信息 */
struct InstalledGameInfo {
    ...
    bool isPending = false;  // 是否等待 FrameQueue 显示
};
