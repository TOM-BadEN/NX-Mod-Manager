...
class StoreModDetail : public Page, public ShellState {
public:
     ...
     * @param fromModList 下载完成后是否返回本地 ModList
     * @param directFromModList 是否直接从本地 ModList 进入
     */
    StoreModDetail(int modId, std::string gameTid, std::string gameName, std::string gameIconKey, GameManager& gameManager, ReturnCallback onReturn = nullptr, ModManager* localModManager = nullptr, std::string gameVersion = "...", bool fromModList = false, bool directFromModList = false);
    ~StoreModDetail() override;
    ...
    BRLS_BIND(brls::Label, m_tagType, "storeModDetail/tagTypeText");
    BRLS_BIND(brls::Label, m_tagVersion, "storeModDetail/tagVersionText");
    BRLS_BIND(brls::Label, m_tagGameVersion, "storeModDetail/tagGameVersionText");
    BRLS_BIND(brls::Label, m_tagSize, "storeModDetail/tagSizeText");
    BRLS_BIND(brls::Label, m_tagAuthor, "storeModDetail/tagAuthorText");
    BRLS_BIND(brls::ScrollingFrame, m_scroll, "storeModDetail/scroll");
    BRLS_BIND(brls::Box, m_scrollContent, "storeModDetail/scrollContent");
    BRLS_BIND(ScrollHint, m_scrollHint, "storeModDetail/scrollHint");
    BRLS_BIND(brls::Label, m_descBody, "storeModDetail/descBody");

    // 中卡片（截图 + 按钮）
    BRLS_BIND(brls::Box, m_middleCard, "storeModDetail/middleCard");
    BRLS_BIND(SkeletonView, m_screenshotSkeleton1, "storeModDetail/screenshotSkeleton1");
    BRLS_BIND(brls::Image, m_screenshot1, "storeModDetail/screenshot1");
    BRLS_BIND(SkeletonView, m_screenshotSkeleton2, "storeModDetail/screenshotSkeleton2");
...
private:
    ...
    brls::GenericEvent::Subscription m_focusChangedSubscription; // 全局焦点变化订阅
    ...
    bool m_fromModList = false;                       // 下载完成后是否返回本地 ModList
    bool m_directFromModList = false;                 // 是否直接从本地 ModList 进入
    ...
    /** @brief 根据返回数量和上一页类型选择动画并返回 */
    void popFromDetail(int popCount);

    /** @brief 左卡片初始化（图标主题、flexWrap、描述滚动、导航） */
    void setupLeftCard();

    /** @brief 根据焦点和描述高度更新左卡片底部滚动提示 */
     ...
     * @param image 解码后的截图
     */
    void applyScreenshot(int index, webpDecoder::WebpImage image);
     ...
     * @param gameTid 游戏 TID
     */
    void loadGameMetadata(int gameIndex, const std::string& gameTid);
