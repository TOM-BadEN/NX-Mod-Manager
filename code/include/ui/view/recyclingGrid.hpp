 ...
 * 移植自 wiliwili 项目，移除了 ButtonRefresh 等特有依赖，添加了轻量骨架和内置翻页功能，
 * 修改了部分代码逻辑
...
public:
    ...
    std::string reuseIdentifier; // Cell 复用类型标识

    /** @brief Cell 即将被重新绑定数据时调用 */
    virtual void prepareForReuse() {}

    /** @brief Cell 进入复用队列时调用 */
    virtual void cacheForReuse() {}

    /** @brief 是否触发数据源选择回调 */
    virtual bool canTriggerSelection() const { return true; }

private:
    size_t index = 0; // Cell 对应的数据索引
};
...
public:
    ...
    bool canTriggerSelection() const override { return false; }

    static RecyclingGridItem* create();

    /** @brief 设置快速骨架模式：透明、不绘制任何内容 */
    void setFastMode(bool fast) { m_fast = fast; }

private:
    bool m_fast = false; // 快速骨架模式
    NVGcolor m_background = brls::Application::getTheme()["app/skeleton"];
};
...
private:
    RecyclingGrid* m_recycler; // 所属回收网格，不持有所有权
};
...
public:
    ...
    /**
     * @brief 重新加载指定索引的可见 Cell
     * @param index 索引
     */
    void reloadItem(size_t index);
    ...
    /**
     * @brief 设置快速骨架模式
     * 骨架透明、不可聚焦（索引 0 作为焦点锚除外），开启时自动锁定交互，
     * 第一个真实卡片出现时自动解锁；关闭时恢复标准骨架并解除锁定。
     * 可在运行中切换，切换后需通过 reloadData 使现有单元格生效。
     */
    void setFastSkeletonMode(bool fast);
    ...
    // 公开配置
    float estimatedRowSpace  = 20;    // 预估行间距
    float estimatedRowHeight = 240;   // 预估行高
    int spanCount            = 4;     // 每行 Cell 数量
    int preFetchLine         = 1;     // 可视区域外预加载行数
    bool isFlowMode          = false; // 是否使用单列不等高布局

private:
    RecyclingGridDataSource* m_dataSource = nullptr; // 数据源，由 RecyclingGrid 持有
    bool m_layouted                       = false;   // 是否已完成首次布局
    float m_oldWidth                      = -1;      // 上一次布局宽度
    bool m_requestNextPage                = false;   // 是否已经请求下一页
    bool m_interactionLocked              = false;   // 是否锁定全部交互
    bool m_fastSkeletonMode               = false;   // 快速骨架模式

    uint32_t visibleMin = 0, visibleMax = 0; // 当前可见 Cell 索引范围
    size_t m_defaultCellFocus = 0;           // 默认焦点 Cell 索引
    int m_deferredFocusIndex  = -1;          // 延迟刷新后的目标焦点

    float m_paddingTop    = 0;     // 顶部内边距
    float m_paddingRight  = 0;     // 右侧内边距
    float m_paddingBottom = 0;     // 底部内边距
    float m_paddingLeft   = 0;     // 左侧内边距
    bool m_paddingPercentage = false; // 左右内边距是否为百分比

    std::function<void()> m_nextPageCallback = nullptr;             // 下一页加载回调
    std::function<void(size_t)> m_focusChangeCallback = nullptr;     // 焦点索引变化回调

    RecyclingGridContentBox* m_contentBox = nullptr; // 承载当前 Cell 的内容容器
    brls::Rect m_renderedFrame;                       // 当前已创建 Cell 的覆盖范围
    std::vector<float> m_cellHeightCache;             // 流式布局 Cell 高度缓存
    std::map<std::string, std::vector<RecyclingGridItem*>*> m_queueMap; // Cell 复用队列
    std::map<std::string, std::function<RecyclingGridItem*(void)>> m_allocationMap; // Cell 工厂
    ...
    /**
     * @brief 设置指定 Cell 的布局信息
     * @param cell 目标 Cell
     * @param index 索引
     * @return Cell 高度
     */
    float layoutCellAt(RecyclingGridItem* cell, size_t index);
     ...
     * @param item 目标 Cell
     */
    void applyInteractionLockedToItem(RecyclingGridItem* item);
};
