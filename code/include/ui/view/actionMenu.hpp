...
struct MenuItemConfig {
    ...
    AfterAction afterAction = AfterAction::CloseMenu;                // 执行完成后的菜单行为
...
public:
    ...
    /**
     * @brief 查询异步任务运行状态
     * @return 异步任务运行中返回 true，否则返回 false
     */
     ...
     * @brief 查询多选状态
     * @param index 选项索引
     * @return 选项已选中时返回 true，否则返回 false
     */
...
private:
    ...
    bool m_asyncRunning = false; // 是否有异步任务运行中
    size_t m_asyncIndex = 0;     // 当前异步任务对应的选项索引
    int m_animFrame = 0;         // 异步加载动画帧计数

    /** @brief 异步动画循环（仅负责加载动画，token 取消后自动停止） */
    void animLoop();
