...
public:
     ...
     */
    StoreModDetail(int modId, std::string gameTid, std::string gameName, GameManager& gameManager, ReturnCallback onReturn = nullptr, ModManager* localModManager = nullptr);
    ~StoreModDetail();
...
private:
    ...
    // 异步任务
    util::AsyncFurture<void> m_detailTask;           // 加载模组详情
    util::AsyncFurture<void> m_screenshotTask;       // 加载截图
    util::AsyncFurture<void> m_commentTask;          // 加载留言
    util::AsyncFurture<void> m_voteTask;             // 点赞/点踩
    util::AsyncFurture<void> m_downloadTask;         // 下载模组
