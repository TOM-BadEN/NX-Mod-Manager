...
private:
    ...
    std::atomic<bool> m_sizeLoading{false};  // 体积计算是否进行中（声明在 loader 之前，确保析构顺序安全）
    std::atomic<int> m_focusedIndex{0};      // 当前焦点索引（体积计算优先级）
    util::AsyncFurture<void> m_installTask;  // 安装/卸载异步任务
    util::AsyncFurture<void> m_iconLoader;  // 异步图标重试任务
    ...
    MenuPageConfig m_editMenu;               // 修改内容子菜单
    MenuPageConfig m_typeMenu;               // 类型选择子菜单
    MenuPageConfig m_manageMenu;             // 管理模组子菜单
    MenuPageConfig m_transferMenu;           // 文件传输子菜单
    MenuPageConfig m_storeMenu;              // 模组商店菜单
