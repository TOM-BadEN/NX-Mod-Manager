...
private:
    ...
    bool m_crcLoading = false;               // CRC32 计算是否进行中
    ...
    /** @brief 检查当前游戏的 MOD 安装条件，失败时弹出提示 */
    bool checkBeforeModInstall();
    ...
    /** @brief 链式触发 CRC32 计算：主线程选最近 mod，提交一个计算任务，完成后自动触发下一个 */
    void submitNextCrc();

    /** @brief CRC32 全部就绪后异步检查模组更新，结果通过手术式 setMod 刷到可见卡片 */
    ...
    /** @brief 怪猎首次进入公告，需要显示时返回 true */
    bool showMHRiseFirstLaunchDialog();
