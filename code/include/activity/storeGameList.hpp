...
private:
    ...
    int m_pendingIcons = 0;                             // 当前正在处理的 icon 数量
    ...
    /** @brief 提交 N 个离焦点最近的图标处理任务 */
    void submitNextIcons(int count);

    /** @brief 给当前可见 Cell 应用图标纹理 */
    void applyVisibleIcon(int index, int textureId);

    /** @brief 用 WebP 数据创建 NVG 纹理 */
    int createIconTexture(const std::vector<uint8_t>& data);

    /** @brief 主线程回调：处理本地图标读取结果，并继续网络验证 */
    void onLocalIconLoaded(int gameIdx, std::string tid, std::vector<uint8_t> data);

    /** @brief 提交单个 icon 网络验证任务 */
    void submitIconNetwork(int gameIdx, std::string tid, api::game::IconCacheValidator validator);

    /** @brief 主线程回调：处理分页加载结果 */
    void onPageLoaded(api::game::GameListResult result);

    /** @brief 主线程回调：处理单个 icon 下载结果 */
    void onIconDownloaded(int gameIdx, api::game::IconResult result, bool savedToFile);
};
