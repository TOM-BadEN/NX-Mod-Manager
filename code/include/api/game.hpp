...
struct GameList {
    ...
    std::string iconKey;     // 纹理缓存 Key
    bool installed = false; // 本机是否已安装该游戏
    bool isPending = true;   // 是否正在显示轻量骨架
    bool isLoading = false;  // 是否已经进入图标加载流程
};
...
/**
 * @brief 获取已安装/未安装游戏列表（POST 请求，带 TID 过滤）
 * @param page 页码（从 1 开始）
 * @param limit 每页数量
 * @param keyword 搜索关键词（空字符串表示不搜索）
...
/** @brief 图标缓存验证信息 */
struct IconCacheValidator {
    bool hasLocalFile = false; // 本地 WebP 文件是否存在
    std::string etag;          // 本地缓存记录的 ETag
    std::string lastModified;  // 本地缓存记录的 Last-Modified
};

/** @brief 图标下载结果 */
struct IconResult : api::ApiResult {
    bool notModified = false;   // 服务端图标未变化，本地 WebP 仍可用
    bool hasData = false;       // 是否带有新的 WebP 数据
    std::vector<uint8_t> data;  // WebP 原始数据
    std::string etag;           // 响应头中的 ETag
    std::string lastModified;   // 响应头中的 Last-Modified
};

/**
 * @brief 下载游戏图标（镜像加速优先，支持 ETag / Last-Modified 条件请求）
 * @param gameTid 游戏 TID
 * @param validator 本地缓存验证信息
 * @param token 用于取消请求的停止令牌
 * @return IconResult 包含图标数据、未变化状态和缓存元数据
 */
