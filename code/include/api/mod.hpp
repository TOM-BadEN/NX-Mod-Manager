 ...
 */
ModListResult fetchModList(const std::string& gameTid, int page = 1, int limit = 20, const std::string& sort = "latest", const std::string& keyword = "", const std::string& version = "", const std::string& modType = "", std::stop_token token = {});

/** @brief 获取模组适配游戏版本列表的结果 */
struct ModGameVersionsResult : api::ApiResult {
    std::vector<std::string> versions; // 版本号列表（"0" 通用版本由客户端手动添加）
};

/**
 * @brief 获取指定游戏下模组所适配的游戏版本列表
 * @param gameTid 游戏 TID
 * @param token 用于取消请求的停止令牌
