...
public:
     ...
     * @param idx 已安装游戏索引
     * @return 已安装游戏的 NACP 元数据，索引无效时返回空结果
     */
    ...
    /**
     * @brief 消费待聚焦游戏 appId
     * @return 待聚焦游戏 appId，没有待处理游戏时返回 0
     */
    uint64_t consumePendingFocus();

    /**
     * @brief 设置待移除游戏（ModList 移除最后一个 mod 后设置，Home onResume 时消费）
     * @param appId 游戏 appId
     */
    ...
    /**
     * @brief 消费待移除游戏 appId
     * @return 待移除游戏 appId，没有待处理游戏时返回 0
     */
...
private:
    ...
    uint64_t m_pendingFocusAppId = 0;                // 待聚焦游戏 appId
    uint64_t m_pendingRemoveAppId = 0;               // 待移除游戏 appId
