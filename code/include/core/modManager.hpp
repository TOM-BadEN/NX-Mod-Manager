...
public:
    /** @brief 管理器外模组提取结果 */
    struct UnmanagedModExtractResult {
        bool success = false;          // 是否全部提取成功
        bool rollbackFailed = false;   // 失败后是否发生回滚失败
        std::string errorPath;         // 失败的源路径或目标路径
    };
     ...
     * @return 是否存在管理器外模组
     */
    bool hasUnmanagedMods() const;

    /**
     * @brief 按已生成的 Plan 提取管理器外模组
     * @return 管理器外模组提取结果
     */
    UnmanagedModExtractResult extractUnmanagedMods();

    /**
     * @brief 获取当前游戏的 MOD 适配类型
     * @return 当前游戏的 MOD 适配类型
     */
    ModGameType modGameType() const;
    ...
    /**
     * @brief 更新 mod 文件 CRC32 到数据 + JSON 缓存
     * @param index mod 索引
     * @param crc32 8 位十六进制小写 CRC32 字符串
     */
    void setFileCrc32(int index, const std::string& crc32);

    /**
     * @brief 构建更新检查用的 JSON（主线程调用）
     * @return JSON 字符串，格式 {"123":"a1b2c3d4",...}，无数据返回空串
     */
    std::string buildUpdateCheckJson() const;

    /** @brief 保存 JSON 缓存 */
    void saveJson();

    /**
     * @brief 商店下载后将 mod 追加到列表末尾，不触发排序
     * @param info mod 信息
     */
    void addModFromStore(ModInfo info);

    /**
     * @brief 商店更新：保留原索引、旧目录和旧 zip 名，替换 zip 内容和元数据
     * @param index mod 索引
     * @param info 新 mod 信息
     * @param tempZipPath 下载完成的临时 zip 路径
     * @return 是否更新成功
     */
    bool updateModFromStore(int index, ModInfo info, const std::string& tempZipPath);
    ...
    /**
     * @brief 删除 mod（删除文件、清 JSON、从列表移除）
     * @param idx mod 索引
     */
    void deleteModFromModList(int idx);
     ...
     * @return 删除任务原始结果
     */
    fs::RemoveResult deleteModContents(int idx, std::function<void(int, int, const char*)> onProgress);
     ...
     * @return 模组安装结果
     */
    ModInstaller::InstallResult installMod(int index, std::function<void(const ModInstaller::Progress&)> progressCb = nullptr, std::stop_token token = {});
     ...
     * @return 模组卸载结果
     */
    ModInstaller::UninstallResult uninstallMod(int index, std::function<void(const ModInstaller::Progress&)> progressCb = nullptr);

    /**
     * @brief 设置 mod 安装状态并保存 JSON
     * @param index mod 索引
     * @param installed 是否已安装
     */
    void setInstalled(int index, bool installed);

    /** @brief 清理所有 mod 安装状态并保存 JSON */
...
private:
    ...
    std::vector<std::string> m_unmanagedCheatTids; // 存在管理器外金手指内容的 TID
    std::vector<std::string> m_unmanagedOtherTids; // 存在管理器外其他内容的 TID
