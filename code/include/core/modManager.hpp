...
public:
    ...
    /**
     * @brief 检查已安装商店模组是否有更新（同步，需在后台线程调用）
     * @param gameTid 游戏 TID
     * @param modsJson JSON 字符串，buildUpdateCheckJson() 的返回值
     * @param token 取消令牌
     * @return 有更新的 mod ID 列表
     */
    std::vector<int> checkForUpdates(const std::string& gameTid, const std::string& modsJson, std::stop_token token);
    ...
    /**
     * @brief 商店更新：保留旧目录和旧 zip 名，替换 zip 内容和元数据
     * @param index mod 索引
     * @param info 新 mod 信息
