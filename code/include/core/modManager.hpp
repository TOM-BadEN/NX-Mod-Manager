...
public:
    ...
    /**
     * @brief 移除 mod（移到中转站、清下载记录、清 JSON、从列表移除）
     * @param idx mod 索引
     */
...
private:
    /** @brief 从所有已安装 MOD 收集涉及的 TID（去重），缺失字段时回退到游戏 TID */
    std::vector<std::string> collectInstalledTidDirs();
