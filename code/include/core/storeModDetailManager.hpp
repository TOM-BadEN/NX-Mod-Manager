...
public:
    ...
    /**
     * @brief 商店下载：创建 mod 目录、移动 zip、写 modInfo.json、写 DownloadRecord
     * @param gameDir 游戏目录路径
     * @param modDirName mod 目录名
     * @param tempZipPath 临时 zip 文件路径
     * @return 构造好的 ModInfo
     */
    ModInfo installMod(const std::string& gameDir, std::string modDirName, const std::string& tempZipPath);
