...
namespace strSort {
    ...
    void sortAZ(std::vector<T>& items, GetName getName) {
        ...
        std::sort(items.begin(), items.end(), [&](const T& a, const T& b) {
            return cache[std::invoke(getName, a)] < cache[std::invoke(getName, b)];
        });
    }
    ...
    void sortAZ(std::vector<T>& items, GetName getName, GroupBy groupBy) {
        std::unordered_map<std::string, std::string> cache;
        for (const auto& item : items) {
            const auto& name = std::invoke(getName, item);
            cache.emplace(name, pinYinCvt::getSortKey(name));
