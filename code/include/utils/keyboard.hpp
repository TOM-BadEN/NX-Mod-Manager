 ...
 * @param initial 预填文本
 * @param maxLen 最大字符数
 * @return 用户输入，取消或创建失败时返回空字符串
 */
inline std::string showText(const std::string& header = "", const std::string& guide = "", const std::string& initial = "", int maxLen = 32) {
    SwkbdConfig kbd;
 ...
 * @return 用户输入，取消或创建失败时返回空字符串
 */
inline std::string showNumber(const std::string& header = "", const std::string& initial = "", int maxLen = 10) {
    SwkbdConfig kbd;
    if (R_FAILED(swkbdCreate(&kbd, 0))) return "";
