...
/** @brief 自动拷贝到栈上、可安全传给 libnx IPC 的路径包装 */
struct FsPath {
    static constexpr int MAX_LEN = 0x301;  // 与 libnx FS_MAX_PATH 一致
    char s[MAX_LEN] = {};                  // 以空字符结尾的路径缓冲区
...
namespace fs {
    ...
    /**
     * @brief 递归检查目录内是否存在未被忽略的文件
     * @param path 目录路径
     * @param ignoredNames 需要忽略的文件名
     */
    ...
    struct DirEntry {
        ...
        bool isFile;           // 是否为文件
        int64_t fileSize = 0;  // 文件大小，目录固定为 0
    };
    ...
    /** @brief 删除任务扫描得到的单个目录及其条目 */
    struct DirCollection {
        std::string path;              // 目录完整路径
        std::vector<DirEntry> entries; // 目录下的直接子条目
    };

    /** @brief 带进度目录删除任务的格式化结果 */
    struct RemoveResult {
        enum Status {
            Completed, // 删除完成
            Cancelled, // 用户取消
        ...
        Status status;         // 任务状态
        int deletedCount;      // 已删除文件数量
        int totalCount;        // 扫描到的文件总数
        std::string errorPath; // FsError 时：失败的文件或目录完整路径
        std::string errorCode; // FsError 时：libnx Result 十六进制字符串
        std::string elapsed;   // Completed 时：格式化后的耗时文本
    };
     ...
     */
    RemoveResult removeDirContentsWithProgress(const FsPath& path, std::stop_token token, std::function<void(int deleted, int total, const char* fileName)> onProgress);
    ...
    private:
        FsFile m_handle{};    // libnx 文件句柄
        int64_t m_offset = 0; // 当前读取偏移
        int64_t m_size = 0;   // 文件总大小
        bool m_open = false;  // 句柄是否已打开
    };
